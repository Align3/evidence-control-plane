...
def test_a_reference_to_an_undefined_story_is_reported(tmp_path):
    ...
    """
    fixture = Path(__file__).parent / "fixtures" / "dangling_story"
    m = build_matrix(
        repo_root=tmp_path, docs_dir=fixture / "docs", features_dir=None, node_ids=[]
    )
    assert subjects(m, "UNKNOWN_STORY_REF") == {"EV-97", "EV-98"}
    assert severity_of(m, "UNKNOWN_STORY_REF", "EV-97") is Severity.HIGH


def test_a_story_defined_without_satisfies_is_not_dangling(tmp_path):
    ...
    assert "EV-91" not in subjects(m, "UNKNOWN_STORY_REF")
    assert "EV-91" not in m.stories, "the precondition this test exists to cover"
    assert "EV-90" not in subjects(m, "UNKNOWN_STORY_REF")
