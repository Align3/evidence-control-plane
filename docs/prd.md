...
**Note on AR-027 / AR-028 and their scenarios.** Both requirements, and the scenarios AR-S-007 and AR-S-008 that demonstrate them, were written by EV-25 when it found A-02 and A-09 resting on citations that did not state their claims. They are claimed here rather than there because both are conditions on **assertion selection** — each scenario's `When` is "the attestation is generated", which is this service and nothing earlier. EV-12 supplies the boundary validity interval AR-027 reads and EV-16 supplies the confirmation records AR-028 reads, but neither story can generate an attestation, so neither can discharge these scenarios; listing them against EV-12 would convert them into a defect the moment EV-12 merged. Until this story lands they are correctly reported as untested scenarios owned by an unlanded story.
...
**Depends on:** EV-22
**Satisfies:** QA-018
**Acceptance:** the generated traceability matrix reports zero critical findings — every assertion in the closed catalogue resolves to a requirement that states its claim and carries a scenario demonstrating it. Stated as prose because the two scenarios written here belong to EV-17.
