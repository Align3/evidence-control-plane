...
**AG-008 stands unchanged, and there is no CI-only merge path for documentation.** No diff to `docs/` merges on green CI and the author's disclosure alone — not a typo fix, not a broken link, not a change that looks editorial to the person who wrote it. A different model reads every documentation change and forms its own view of whether the project should be bound by it. Whether the text is genuinely non-testable, whether it is genuinely additive, and whether it is genuinely immaterial are **findings the reviewer reaches**, not certifications the author supplies to unlock a merge. That distinction is the whole of this clause: a semantic claim asserted by the author of the change is a self-report, and §5 opens by rejecting self-reports as evidence.

**Two earlier drafts of this clause were wrong in the same way, recorded because the error is a natural one to repeat.** The first allowed CI-only self-merge for any change that was additive and non-testable, treating those two properties as mechanical conditions. They are not: both are semantic assertions, and CI can establish neither. Review falsified the draft with a rule that satisfied every condition it named — *a security fix must sit public and unmodified for thirty days before it may merge* — which is additive, non-testable, orphan-free, confined to `docs/`, and obviously something no project should adopt without a second reader.

The second draft kept a narrower CI-only path for **editorial** changes — no normative and no decision-making delta — fenced by a requirement-ID check and a list of words carrying normative force. Review falsified that too, with a smaller change: strike `EV-16` from AG-013 and make *those same four stories* three. It renumbers no requirement ID, contains none of the listed words, touches only `docs/`, and leaves the scenario count and the orphan count exactly where CI found them — while removing dual review from one of the two stories whose failure mode this product exists to prevent. Normative force lives in meaning, not in vocabulary: a numeral and a list membership carry it as readily as a MUST, and every counterexample answered by extending the word list only buys a longer list. The deeper fault is that **deciding a change is immaterial is itself the semantic reading**, so it cannot be the precondition for skipping one — an author who has missed the delta will report none, and the guard passes precisely when it is needed. Restricting the exemption to particular sections would not close it either: harmful governance edits can be written anywhere.

What survives is the narrow distinction this clause opened with. A mechanical condition can retire the **reproduction session**, because reproduction is about whether commands would produce anything a reviewer has not already seen, and CI can answer that. No mechanical condition retires the **reading**, because every proxy for "this text does not change what the project is bound by" is a proxy for a semantic property, and the only instrument that measures a semantic property is a semantic reader.

Claiming the AG-009a exemption is disclosed in the PR under AG-015, stating the ground for it. An exemption taken silently is indistinguishable from a review that was skipped.
