...
Rationale for requirements originated under this rule — the rejected alternatives obligation (2) requires — is recorded in `docs/decision-log.md`, together with the running list of what has been originated and its confirmation status. **A cold-build prompt MUST exclude that file, and MUST NOT reference it.** Obligation (2) and obligation (4) otherwise defeat each other: a rationale explaining why the rejected shape was wrong hands the next implementer the answer, the mechanism and the fix, and whatever they build then avoids the defect because the text told them to rather than because they derived it. Keeping the rationale out of the normative documents lets both obligations hold at once, and makes the exclusion a standing rule rather than a judgement to be reconstructed for each prompt.

This was not foreseen when AG-018 was written. It was found by the first cold build, which reported that its own assigned reading had defeated the experiment before it wrote a line of code.
