...
> **`capped_by_class` is added here as a decision, and it is provisional (AG-018).** ES-018 has required the member since it was written, and this list omitted it — so did the normative vector, the Python writer, and the Go verifier. Adding it to the enumeration makes those four artifacts agree with ES-018 rather than with each other. **It is required from `schema_version` 2.0.0 and optional by presence at 1.0.0** (ES-002b: encode absence by omission, never as `null`). Requiring it at 1.0.0 would refuse every attestation already issued and the normative vector along with them, which ES-028 classes as a breaking change needing a major version — so the obligation attaches at the version reserved for exactly that in ES-035. **Rejected alternative:** requiring it at 1.0.0 and reissuing the corpus, which buys immediate uniformity at the cost of invalidating history the specification promises stays verifiable.
...
- **Rounding truncates toward zero.** `2/3` encodes as `"0.6666"`, not `"0.6667"`. This follows the convention ES-030 already sets for `clock_skew_ms`, and it is the only direction that cannot overstate: a ratio rounded up reports more coverage than was measured, which is the failure this document exists to prevent. *(Decision under AG-018; rejected alternative: round-half-up, conventional in reporting and marginally more accurate, but it can round a measured shortfall away and produces a number very slightly larger than the evidence supports.)*
- **A zero denominator yields `null`, not zero.** Where `population − out_of_scope` is zero there is no ratio to state, and `0.0000` would assert that nothing in a population was matched rather than that no population existed. *(Decision under AG-018; rejected alternative: emitting `"0.0000"`, which is arithmetically defensible and reads as total failure of coverage to anyone who does not check the counts.)*
...
**`capped_by_class` is true if and only if `class_admissible_level < evidence_supported_level`.** The comparison is strict: where the two are equal the flag is **false**. The field answers one question — *would a stronger denominator class have produced a higher claim?* — and only a class strictly below what the evidence supports makes that answer yes. Where evidence and ceiling coincide, the evidence is what limited the claim and it merely happened to land on the ceiling; reporting that as capped is not quite false, but it answers a different question than the one the field exists to answer, and it would fire the flag on a case where improving the denominator class changes nothing.

**A verifier can check this in one direction only, and MUST NOT claim the other.** Where the claimed level is strictly below the class-admissible level, `min()` forces the evidence-supported level to equal the claim, so the flag is determinately `false` and a record asserting `true` MUST be rejected. Where the claimed level sits exactly at the ceiling, the flag depends on `evidence_supported_level`, which no requirement derives from the records a bundle carries: `coverage-methodology.md` §5 defines the levels in prose but nothing maps record presence to one. A verifier MUST report that case as unchecked rather than resolve it in either direction. Closing that gap requires a record-to-level derivation rule that does not yet exist.
...
Until now nothing defined this. TM-S-004 says "it validates an attestation bundle", QA-008 requires bundles to be byte-identical, SE-021 scopes buyer access to "the evidence explicitly included in its bundle", and `data-model.md` DM-023 deferred the reassembly to EV-19 — but no requirement stated a container, and no vector carried one.

**Why an array rather than named members.** A keyed container — `attestation`, `population_records`, `gap_records` — reads better and was built first. It is wrong, and the reason was found by building it rather than by reasoning about it: a keyed container states each record's role *independently of the record*, so the two can disagree. A validly signed `CoverageGap` placed under `population_records` parsed, authenticated, and reached the coverage engine as a population — counted in the wrong place, and simultaneously absent from the gap set CM-014 conservation is checked against. Every signature on that bundle was valid. It is a gap-hiding path assembled entirely out of well-formed records, which is the exact overclaim class this specification exists to prevent. An array cannot express it, because there is no filing to get wrong.

**Why canonical.** Each record is recovered from the container as a byte range, and ES-001 checks that range against the canonical form. A free-form container would present every record it carries as non-canonical, leaving only two ways out: re-serialize the records, which destroys the bytes the signature covers, or stop checking ES-001 inside a bundle. Requiring the container to be canonical makes QA-008's byte-identical bundle a property of the whole artifact rather than of its parts.
